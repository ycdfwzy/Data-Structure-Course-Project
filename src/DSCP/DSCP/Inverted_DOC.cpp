...
Inverted_DOC_Node* Inverted_DOC::Insert(wCharString &Term, int TermID, int DocID, int Times) {
	...
	while (x != NIL) {
		y = x;
		if (x->Term == Term) {			//如果该节点已存在，将此文档插入到该节点下的文档链表里
...
void Inverted_DOC::Remove(Inverted_DOC_Node *z) {
	...
	if (z->son[0] == NIL || z->son[1] == NIL)	//无左右儿子
		y = z;
	else y = successor(z);
	if (y->son[0] != NIL)
		x = y->son[0];
...
void Inverted_DOC::Edit(wCharString &c, int DocID, int Times) {
	Inverted_DOC_Node *tmp = search(c);
	if (tmp == NIL) {//节点不存在  直接插入
		Insert(c, 0, DocID, Times);
	}
	else//节点存在  编辑节点
	{
		tmp->Edit(DocID, Times);
	}
}
