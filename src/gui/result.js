...
query.split_words(parsed.query, (err, res) => {
    ...
    let element_of_doc = (doc) => {
        return `<h3 class="result-title"><a href="${doc.url}" onclick="openExtern(this, event);">${cvt(doc.headline)} - ${doc.author}</a></h3>
        ...
        let appendNew = (array) => {
            return () => {
                ...
                subarray.forEach((x) => {
                    ...
                    `<div class="search-result" id="${x.id}">
                    </div>`
                    );
                });
                subarray.forEach((x) => {
                    ...
                    query.get_doc(x.id, (err, doc) => {
                        $('#'+x.id).append(element_of_doc(doc));
